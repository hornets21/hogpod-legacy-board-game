...
export function findAvailableNpcCell(state) {
  ...
  for (let cell = 2; cell <= 89; cell++) {
    if (monsterCells.has && monsterCells.has(cell)) continue;
  ...
  if (pool.length === 0) return Math.floor(Math.random() * 80) + 5;
...
export function swapPlayerSkill(state, playerIndex, oldSkillId, newSkill) {
  ...
  const newSkills = player.skills.map((s) => (s.id === oldSkillId ? newSkill : s));
  const updatedPlayers = [...state.players];
  updatedPlayers[playerIndex] = { ...player, skills: newSkills };

  const log = [...state.log, `🧙‍♂️ ${player.name} สลับสกิลเป็น "${newSkill.nameTh || newSkill.name}" เรียบร้อยแล้ว!`];
  const marquee = [...(state.marqueeAnnouncements || []), `📢 ${player.name} เปลี่ยนสกิลเป็น "${newSkill.nameTh || newSkill.name}" กับ NPC ผู้ฝึก Skill`];

  let newState = despawnNpc({ ...state, players: updatedPlayers, log, marqueeAnnouncements: marquee }, "skill_trainer");
  return newState;
}
...
export function changePlayerPet(state, playerIndex, newPet) {
  ...
  let newState = despawnNpc({ ...state, players: updatedPlayers, log, marqueeAnnouncements: marquee }, "pet_trainer");
  return newState;
}
