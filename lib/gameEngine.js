...
export function movePlayer(state, playerIndex, steps) {
  ...
  // Check Bingo status
  const { updatedPlayer, logs: bingoLogs } = checkPlayerBingo(player, newPos);
...
// ─── NPC Mechanics ───────────────────────────────────────────
export function findAvailableNpcCell(state) {
  const monsterCells = state.monsterCells || new Set();
  const monsterMap = state.monsterMap || {};
  const cellTeleport = state.cellTeleport || {};
  const trapCells = state.trapCells || {};
  const npcs = state.npcs || {};

  const hasMonster = (cell) => {
    if (monsterMap[cell]) return true;
    if (monsterCells instanceof Set) return monsterCells.has(cell);
    if (Array.isArray(monsterCells)) return monsterCells.includes(cell);
    return false;
  };

  const npcOccupied = new Set(
    Object.values(npcs)
      .filter((n) => n && n.isSpawned && n.cell)
      .map((n) => n.cell)
  );

  // Determine player positions to spawn NPC in a relevant zone
  const players = state.players || [];
  const playerPositions = players.map((p) => p.position || 0);
  const minPos = playerPositions.length > 0 ? Math.min(...playerPositions) : 0;
  const maxPos = playerPositions.length > 0 ? Math.max(...playerPositions) : 0;

  // Zone range: from slightly ahead of trailing player to ~20 steps ahead of leading player
  const minZoneCell = Math.max(2, minPos + 2);
  const maxZoneCell = Math.min(89, Math.max(minZoneCell + 15, maxPos + 20));
...
export function spawnNpc(state, npcId) {
  ...
  const targetCell = findAvailableNpcCell(state);
  const updatedNpcs = {
    ...(state.npcs || {}),
    [npcId]: {
      id: npcId,
...
export function despawnNpc(state, npcId) {
  ...
  if (npcs[npcId]) {
    npcs[npcId] = {
      ...
      isSpawned: false,
      cell: null,
      cooldown: NPC_CONFIG.SPAWN_COOLDOWN_SECONDS || 180,
    };
  }
  return { ...state, npcs };
}
...
export function handleNpcLanding(state, playerIndex, npcId) {
  ...
  if (npcId === "doctor") {
    ...
    updatedPlayers[playerIndex] = { ...player, potions: currentPotions };

    const grantedNames = granted.length > 0 ? granted.map((g) => g.name).join(", ") : "กระเป๋ายาเต็ม";
    const msg = `🧪 ${player.name} ตกช่อง NPC หมอยา! ได้รับยาปรุงพิเศษ 2 ขวด: ${grantedNames}`;
    log.push(msg);
    marquee.push(`📢 ${player.name} ได้รับยาปรุงพิเศษ 2 ขวดจาก NPC หมอยา!`);
...
export function equipArmorToPlayer(player, newArmor) {
  // Remove stats of old armor
  if (player.armor) {
    if (player.armor.isMaxHpBuff) {
      const oldHpBonus = player.armor.hpBonus || 0;
      player.maxHp = Math.max(BASE_HP, player.maxHp - oldHpBonus);
      if (player.hp > player.maxHp) player.hp = player.maxHp;
    } else if (player.armor.hpBonus) {
      // Revert previous flat HP modification
      player.hp -= player.armor.hpBonus;
    }
  }

  // Apply stats of new armor
  player.armor = newArmor;
  if (newArmor) {
    if (newArmor.isMaxHpBuff) {
      const hpBonus = newArmor.hpBonus || 0;
      player.maxHp += hpBonus;
      player.hp += hpBonus;
    } else if (newArmor.hpBonus) {
      player.hp += newArmor.hpBonus;
    }
  }
...
export function equipAmuletToPlayer(player, newAmulet) {
  // Remove stats of old amulet
  if (player.amulet && player.amulet.hpBonus) {
    player.hp -= player.amulet.hpBonus;
  }

  // Apply stats of new amulet
  player.amulet = newAmulet;
  if (newAmulet && newAmulet.hpBonus) {
    player.hp += newAmulet.hpBonus;
  }
...
export function buyItem(state, playerIndex, itemType, itemId) {
  ...
  if (itemType === "bingo") {
    ...
    const { updatedPlayer, logs: bingoLogs } = checkPlayerBingo(player, player.position);
