...
export function findAvailableNpcCell(state, npcId = null) {
  ...
  // Merchant special tight zone: 1 to 10 steps ahead of current player positions
  let minZoneCell, maxZoneCell;
  if (npcId === "merchant") {
    minZoneCell = Math.max(2, maxPos + 3);
    maxZoneCell = Math.min(89, Math.max(minZoneCell + 15, maxPos + 22));
  } else {
    // Standard zone range for other NPCs:
    minZoneCell = Math.max(2, minPos + 2);
    maxZoneCell = Math.min(89, Math.max(minZoneCell + 15, maxPos + 20));
  }
