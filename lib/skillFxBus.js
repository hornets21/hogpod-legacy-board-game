...
export function emitSkillCast({ playerId, skillId, targetIndex = null, skillData = null, visualContext = null }) {
  emit(FX_EVENTS.SKILL_CAST, { playerId, skillId, targetIndex, skillData, visualContext, ts: Date.now() });
}

export function emitDamageDealt({ targetIndex, amount, type = "skill", sourceId = null, visualContext = null }) {
  emit(FX_EVENTS.DAMAGE_DEALT, { targetIndex, amount, type, sourceId, visualContext, ts: Date.now() });
}
