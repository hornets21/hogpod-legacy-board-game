...
export function generateRandomBoardElements() {
  ...
  const commonPool = MONSTERS.filter((m) => !m.isBoss && !m.isElite && !m.isHidden && !m.isHealer);
  const strongPool = MONSTERS.filter((m) => !m.isBoss && (m.isElite || m.isHidden || m.isHealer));

  // แถบช่วงช่องสำหรับมอนสเตอร์ (แบ่งกระดานออกเป็น 2 โซน)
  // โซนต้นกระดาน (ช่อง 2..45) สำหรับมอนสเตอร์ทั่วไป (Common)
  // โซนท้ายกระดาน (ช่อง 46..89) สำหรับมอนสเตอร์เก่งๆ (Elite / Secret / Healer)
  const earlyZone = shuffle(availableCells.filter((c) => c >= 2 && c <= 45));
  ...
  const dynamicMonsters = MONSTERS.map((m) => {
    ...
    let cell;
    if (m.isElite || m.isHidden || m.isHealer) {
