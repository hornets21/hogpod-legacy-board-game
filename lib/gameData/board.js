...
export function generateRandomBoardElements() {
  ...
  // Keep the full randomized layout as a reserve, but reveal only a small
  // wave at the start. Later waves are activated by the game engine as the
  // party progresses, so the board does not become impossible to clear.
  const shuffledSpawnables = shuffle(dynamicMonsters.filter((m) => !m.isBoss));
  const openingCandidates = shuffledSpawnables.filter((m) => m.cell >= 11 && m.cell <= 45);
  const openingMonsters = openingCandidates.slice(0, 6);
  const openingIds = new Set(openingMonsters.map((m) => m.id));
  const activeMonsters = [
    ...openingMonsters,
    ...dynamicMonsters.filter((m) => m.isBoss),
  ];
  const monsterReserve = shuffledSpawnables.filter((m) => !openingIds.has(m.id));
  const monsterMap = {};
